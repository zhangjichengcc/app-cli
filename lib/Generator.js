...
class Generator {
  ...
  getTemplateType(generatorType) {
    const templates = fs
      .readdirSync(`${__dirname}/generators/${generatorType}/templates`)
      .filter((f) => !f.startsWith('.'))
      .map((f) => {
        return {
          name: `${f.padEnd(15)} - ${chalk.gray(require(`./generators/${generatorType}/templates/${f}/meta.json`).description)}`,
          value: f,
          short: f,
        };
      });
